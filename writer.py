    ...
    def update(self):
        ...
        while True:
            ...
            if orig_img is None:
                ...
                if self.save_video:
                    stream.release()
                video_name = os.path.splitext(os.path.basename(self.opt.video))[0]
                output_file_name = f'alphapose-results-{video_name}.json'
                write_json(final_result, self.opt.outputpath, form=self.opt.format, for_eval=self.opt.eval, outputfile = output_file_name)
